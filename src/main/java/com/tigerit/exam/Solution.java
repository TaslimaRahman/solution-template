...
public class Solution implements Runnable {
    ...
    public void run() {
        ...
        Integer table_num = 0;
        String[] table_name = new String[100];
        int colNum = 0;
        int rowNum = 0;
        int length = 0;
        String[] sentence = new String[100];
        String[] word = new String[100];
        String[][] colName = new String[100][100];
        double[][][] data = new double[100][100][100];
        double[][][] new_data = new double[100][100][100];
        String[][] dum_column = new String[100][100];
        int queryNum = 0;
        String[][] query = new String[100][100];
        StringBuilder[] dum_sentence = new StringBuilder[100];
        String[][] dum_data = new String[100][100];
        for (int a = 1; a <= test_case; a++) {
            ...
            table_name = new String[table_num + 1];
            for (int i = 0; i < table_num; i++) {
                table_name[i] = readLine();
                Matcher matcher = pattern.matcher(table_name[i]);
                if (!matcher.find()) {
                ...
                colName[i] = c_input.split(" ");
                for (int x = 0; x < colNum; x++) {
                    Matcher match = pattern.matcher(colName[i][x]);
                    if (!match.find()) {
                        throw new IllegalArgumentException("lower case,numbers or _ are allowed");//only lower case,number and _ allowed
                    }
                }
                ...
                for (int j = 0; j < rowNum; j++) {
                    for (int k = 0; k < colNum / colNum; k++) {
                        dum_data[j][k] = readLine();
                        String[] split_data = dum_data[j][k].split(" ");
                        for (int y = 0; y < colNum; y++) {
                            data[i][j][y] = Double.parseDouble(split_data[y]);
                            if (data[i][j][y] < 0 && data[i][j][y] > 100000) {
            ...
            for (int b = 0; b < queryNum; b++) {
                ...
                dum_sentence[b] = new StringBuilder();
                for (int c = 0; c < 4; c++) {
                    query[b][c] = readLine();
                    dum_sentence[b].append(query[b][c]);
                    dum_sentence[b].append(" ");

                }

                sentence[b] = dum_sentence[b].toString();
                System.out.print("\n");
            }
        ...
        for (int a = 1; a <= test_case; a++) {
            ...
            for (int b = 0; b < queryNum; b++) {
                ...
                word = sentence[b].split(" ");
                for (int w = 0; w < word.length; w++) {
                    if (word[1].equals("*")) {
                        for (int i = 0; i < table_num; i++) {

                            if (word[w].equals(table_name[i])) {
                                for (int col = 0; col < colNum; col++) {
                                    System.out.printf("%s\t", colName[i][col]);
                                }
                            }
                        }

                    } else {
                        if (word[w].equals("FROM")) {
                        break;
                        ...
                        for (int k = 0; k < dum_word.length; k++) {
                            for (int y = 0; y < word.length; y++) {
                                for (int i = 0; i < table_num; i++) {
                                    if (word[y].equals(table_name[i])) {
                                        if (dum_word[k].equals(word[y + 1])) {
                                            for (int j = 0; j < colNum; j++) {
                                                if (colName[i][j].equals(dum_word[k + 1])) {
                                                    System.out.printf("%s\t", colName[i][j]);
                                                    dum_column[i][j] = colName[i][j];
                                                }
                                            }
                                        }
                                    }
                    ...
                    if(word[w].equals("=")){
                     ...
                     //   if(word[1].equals("*")){
                        for(int tab1 = 0; tab1<table_num; tab1++){
                            if(table_name[tab1].equals(word1[0])){
                                for(int col1=0; col1<colNum; col1++){
                                    if(colName[tab1][col1].equals(word1[1])){
                                        for(int tab2=0; tab2<table_num; tab2++){
                                            if(table_name[tab2].equals(word2[0])){
                                                for(int col2=0; col2<colNum; col2++){
                                                    if(colName[tab2][col2].equals(word2[1])){
                        ...
                        for(int w1=0; w1<word.length; w1++){
                          if(word[w1].equals(word1[0])){
                             for(int tab1=0; tab1<table_num; tab1++) {
                                if(table_name[tab1].equals(word[w1-1])){
                                  for(int col1=0; col1<colNum; col1++){
                                    if(colName[tab1][col1].equals(word1[1])){
                                     for(int w2=0; w2<word.length; w2++){
                                       if(word[w2].equals(word2[0])){
                                        for(int tab2=0; tab2<table_num; tab2++){
                                            if(table_name[tab2].equals(word[w2-1])){
                                                for(int col2=0; col2<colNum; col2++){
                                                    if(colName[tab2][col2].equals(word2[1])){
                                                        for(int r1=0; r1<rowNum; r1++){
                                                           for(int r2=0; r2<rowNum; r2++){
